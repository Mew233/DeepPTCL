...
from congfu.layers import BasicLayer, CongFuLayer, GINEConv, create_mlp, BasicLayer_GDSC, CongFuLayer_GDSC
...
class CongFuBasedModel(nn.Module):
    ...
    def _generate_basic_layers(self, number_of_layers: int):
        basic_layers = []
    ...
    def _generate_congfu_layers(self, number_of_layers: int):
        congfu_layers = []
...
class CongFuBasedModel_GDSC(nn.Module):
    '''Class for CongFu model'''
    ...
    def __init__(self,
        ...
        self.context_encoder = create_mlp(context_dim, [feature_dim], emb_dim, activation="relu")
        self.output_transformation = create_mlp(emb_dim, [feature_dim], feature_dim // 2, activation="relu")
        self.mlp = create_mlp(self.emb_dim + int(feature_dim/2), mlp_hidden_dims, 1, activation="leaky_relu")
    ...
    def _generate_basic_layers(self, number_of_layers: int):
        ...
        for i in range(number_of_layers):
            graph_update_gnn = GINEConv(self.emb_dim, self.emb_dim, NUM_BOND_TYPE, NUM_BOND_DIRECTION).to(self.device)
            last_layer = i == number_of_layers - 1
            basic_layer = BasicLayer_GDSC(self.emb_dim, graph_update_gnn, last_layer).to(self.device)
            basic_layers.append(basic_layer)
    ...
    def _generate_congfu_layers(self, number_of_layers: int):
        ...
        for i in range(number_of_layers):
            ...
            congfu_layer = CongFuLayer_GDSC(self.emb_dim, self.emb_dim, self.emb_dim, graph_update_gnn, bottleneck_gnn,
                                       last_layer).to(self.device)
            congfu_layers.append(congfu_layer)
    ...
    def forward(self, graphA: Data, context: torch.Tensor) -> torch.Tensor:
        graphA.context_x_edges = self._create_context_graph_edges(graphA)
        graphA = self._embed_x(graphA)
        context = self.context_encoder(context)
        ...
        for layer in self.congfu_layers:
            graphA, context = layer(graphA, context)

        graphA.x = global_mean_pool(graphA.x, graphA.batch)
        graphA.x = self.output_transformation(graphA.x)

        input_ = torch.concat((graphA.x, context), dim=1)
