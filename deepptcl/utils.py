...
def get_datasets(data_folder_path: str,
    ...
    if test_mode == False:
        ...
        cell_lines = pd.read_feather(data_folder_path + synergy_score + f".feather").set_index("cell_line_name")
...
def get_mol_dict_GDSC(df: pd.DataFrame) -> dict:

    '''Returns the mapping between IDs and molecular graphs'''

    mols = pd.concat([
        df.rename(columns={'Drug_ID': 'id', 'Drug': 'drug'})[['id', 'drug']],
        # df.rename(columns={'Drug2_ID': 'id', 'Drug2': 'drug'})[['id', 'drug']]
    ],
        axis=0, ignore_index=True
    ).drop_duplicates(subset=['id'])
